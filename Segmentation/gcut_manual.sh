...
# run mri_gcut changing T flag thresh between 0-1, 0 = more conservative 1 = more aggressive
thresh=0.3
echo threshold=$thresh
if [ ! -f "$FreesurferDirMRI/brainmask.gcuts.mgz" ]; then
	mri_gcut -110 -mult brainmask.auto.mgz -T $thresh T1.mgz brainmask.mgz brainmask.gcuts.mgz
else
