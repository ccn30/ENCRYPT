...
%load('analysis/coded_full_data_DEBUG.mat');
...
% Initialise variable vectors for extractopm
...
Flag1Time = zeros(nTrials,1);
Flag3Time = zeros(nTrials,1);
TrigTime = zeros(nTrials,1);
...
for triali = 1:nTrials
    ...
    Flag1Time(triali,1) = str2double(coded_full_data.Block.Trials.Trial{1,triali}.Path.Tester_Flag_1_Reached_Timestamp.Text);
    Flag3Time(triali,1) = str2double(coded_full_data.Block.Trials.Trial{1,triali}.Path.Tester_Flag_3_Reached_Timestamp.Text);
    TrigTime(triali,1) = str2double(coded_full_data.Block.Trials.Trial{1,triali}.Path.Tester_Acquired_Position_TimeStamp.Text);
end
...
%% 3. Calculate errors and trial metrics
% errors = absolute distance (AD), absolute angular (AA) and proportional distance/angular errors (PD/PA)
...
DistIncom = zeros(nTrials,1); % Calculate size of triangle (flag 3 to 1 incoming path)
...
for triali = 1:nTrials
    ...
    TimeIncom(triali,1) = TrigTime(triali,1) - Flag3Time(triali,1);
    
    TimeOut(triali,1) = Flag3Time(triali,1) - Flag1Time(triali,1);
...
disp('Errors and metrics Calculated...')
%% 4. Put all above trial variables into table and save

TInfo = table(Code, Block, TrialNo, Type, Env, OoB, OoBPos, Flag1, Flag2, Flag3, Trig, ADerror, PDerror, AAerror, PAerror, DistIncom, DistOut, TimeIncom, TimeOut);

writetable(TInfo,[resultsDir '/PIT_RawResults_' CBcode '.csv']);
disp(' ');
disp(['New results-table: PIT_RawResults_' CBcode '.csv made in ' resultsDir]); 
